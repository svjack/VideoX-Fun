...
# model path
model_name          = "../ComfyUI/models/Fun_Models/Wan2.1-Fun-V1.1-1.3B-InP/"
...
# ... [keep all the original imports and setup code above] ...

# Define the directory containing image-text pairs
image_text_dir = "Dont_be_your_lover_Images_Captioned"

# Get all .png files in the directory
png_files = [f for f in os.listdir(image_text_dir) if f.endswith('.png')]

# Prepare the pairs (png file and corresponding txt file)
file_pairs = []
for png_file in png_files:
    txt_file = png_file.replace('.png', '.txt')
    if os.path.exists(os.path.join(image_text_dir, txt_file)):
        file_pairs.append((png_file, txt_file))

# Process each pair with tqdm progress bar
for png_file, txt_file in tqdm(file_pairs, desc="Generating videos"):
    # Set the image path
    validation_image_start = os.path.join(image_text_dir, png_file)
    validation_image_end = None

    # Read the prompt from the text file
    with open(os.path.join(image_text_dir, txt_file), 'r', encoding='utf-8') as f:
        prompt = f.read().strip()

    # Update the save path to include the original filename
    original_filename = os.path.splitext(png_file)[0]
    current_save_path = os.path.join(save_path, original_filename)
    ...
    if video_length == 1:
        video_path = os.path.join(current_save_path, "output.png")
        image = sample[0, :, 0]
        ...
        video_path = os.path.join(current_save_path, "output.mp4")
        save_videos_grid(sample, video_path, fps=fps)

    # Reset the generator seed for the next iteration to ensure different results
    seed += 1
    generator = torch.Generator(device=device).manual_seed(seed)
